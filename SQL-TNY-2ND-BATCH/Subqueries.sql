...
--Generate a report, which contain the details of those employee who is earning more than the average salary of there
--respective department.

select e.employee_id,e.salary,e.first_name,d.department_name,tbl.avg_Sal
from hr.employees e,
   hr.departments d,
           ( select avg(salary) avg_Sal,department_id
              from hr.employees e
             group by department_id ) tbl
where e.salary < tbl.avg_Sal
and e.department_id = tbl.department_id
...
where not exists (select 1 from hr.employees e1 where e1.department_id = d1.department_id);
-- EXISTS -TRUE when data is there and retrun false when no data is there
-- not exists -- True where no data is there and return false when data is there
select * from hr.departments d1
where  exists (select 1 from hr.employees e1 where e1.department_id = d1.department_id);
...
select distinct hr.departments.*
from hr.employees ,
     hr.departments  
where hr.employees.department_id(+) = hr.departments.department_id;

select e.employee_id,e.salary,e.first_name,d.department_name,tbl.max_sal
from hr.employees e,
   hr.departments d,
           (select max(salary) max_sal,department_id
from hr.employees
group by department_id) tbl
where e.salary = tbl.max_sal
and e.department_id = tbl.department_id
and e.department_id = d.department_id;
...
from hr.employees;
   ...
   row_number() over (order by salary desc) rown
   from hr.employees;
   
    select employee_id,salary,first_name,
   row_number() over (order by salary) rown
   from hr.employees;

 select employee_id,department_id,salary,first_name,
   row_number() over (PARTITION by department_id order by salary) rown
   from hr.employees;
   
   select employee_id,salary,first_name,
   rank() over (order by salary) rown
   from hr.employees;
   
   select employee_id,salary,first_name,
   dense_rank() over (order by salary) rown
   from hr.employees;
